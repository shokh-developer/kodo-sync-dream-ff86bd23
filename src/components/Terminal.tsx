...
<body><div style="padding:24px"><h1>CSS Preview</h1><p>HTML fayl yarating.</p>
...
* { box-sizing: border-box; }
  ...
  const handleServerResult = (data: any) => {
    ...
    if (data.compile) {
      if (data.compile.stderr) addLog("compile", `Kompilyatsiya xatosi:\n${data.compile.stderr}`);
      if (data.compile.stdout) addLog("compile", `Kompilyatsiya:\n${data.compile.stdout}`);
    }
    if (data.run) {
      ...
      else if (data.run.code !== undefined) addLog("warn", `Dastur ${data.run.code} kodi bilan tugadi`);
    }
  };
  ...
  const runCode = async (stdin: string) => {
    setIsRunning(true);
    clearLogs();
    addLog("info", `▶ ${language.toUpperCase()} ishga tushirilmoqda...`);
                      ...
                      {logs.length === 0 ? (
                        <div className="text-muted-foreground/60 flex flex-col items-center justify-center h-full gap-3">
                          ...
                          <div className="text-center">
                            <p className="text-sm font-sans font-medium mb-1">Terminal tayyor</p>
                            <p className="text-xs text-muted-foreground/50 font-sans">
                              Run tugmasini bosing yoki buyruq kiriting
                            </p>
                          </div>
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              ...
                              if (value === 'run') handleRunClick();
                                ...
                                addLog('log', '  node <fayl>     - JavaScript');
                                addLog('log', '  python <fayl>   - Python');
                                addLog('log', '  g++ <fayl>      - C++');
                                addLog('log', '  exec <til> <kod> - Istalgan til');
                                addLog('log', '  ls / cat <fayl> - Fayllar');
                                ...
                                } else addLog('info', 'Fayllar mavjud emas');
                                ...
                                else addLog('error', 'Format: exec <til> <kod>');
                                ...
                                addLog('warn', 'npm/yarn/npx qo\'llab-quvvatlanmaydi.');
                              } else {
                    ...
                    {previewUrl ? (
                      ...
                      <div className="flex items-center justify-center h-full text-muted-foreground/50">
                        <div className="text-center">
                          ...
                          <p className="text-sm font-medium mb-1">Preview bo'sh</p>
