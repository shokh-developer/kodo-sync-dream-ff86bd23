...
const MyRooms = () => {
  ...
  const handleLeaveRoom = async (roomId: string) => {
    ...
    try {
      ...
      toast({ title: "Success", description: "You left the room" });
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-background relative">
      ...
      <div className="container mx-auto px-6 py-10 max-w-3xl relative z-10">
        ...
        <motion.section className="mb-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          <h2 className="text-sm font-medium text-muted-foreground mb-4 flex items-center gap-2 uppercase tracking-wider">
            ...
            Rooms I created ({createdRooms.length})
          </h2>

          {createdRooms.length === 0 ? (
            <div className="glass-card rounded-xl p-8 text-center">
              <p className="text-sm text-muted-foreground mb-4">You have not created any rooms yet</p>
        ...
        <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          <h2 className="text-sm font-medium text-muted-foreground mb-4 flex items-center gap-2 uppercase tracking-wider">
            ...
            Rooms I joined ({joinedRooms.length})
          </h2>
      ...
      <AlertDialog open={!!deleteRoomId} onOpenChange={() => setDeleteRoomId(null)}>
        <AlertDialogContent className="bg-card/95 backdrop-blur-xl border-border/50 rounded-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-foreground font-space">Delete room</AlertDialogTitle>
            <AlertDialogDescription className="text-muted-foreground">
              This room and all files inside it will be deleted. This action cannot be undone.
